...
export const deleteDataSource = ({ id, name }: DeleteDataSourceConfig) => {
  ...
  */
  ...
  e2e.getScenarioContext().then(({ addedDataSources }: any) => {
    ...
    const lastAddedDataSource = addedDataSources[addedDataSources.length - 1];

    e2e.setScenarioContext({
      addedDataSources,
      lastAddedDataSource: lastAddedDataSource?.name ?? '',
      lastAddedDataSourceId: lastAddedDataSource?.id ?? '',
    });
  });
};
